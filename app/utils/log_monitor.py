...
class LogFileMonitor:
    def __init__(self, job_id=None, log_dir="logs"):
        # Prioritize task ID from environment variable
        self.job_id = job_id or os.environ.get("OPENMANUS_TASK_ID")
        self.log_dir = log_dir
        
        # Prioritize log file path from environment variable
    ...
    def start_monitoring(self):
        ...
        # If log file already exists, read existing content
    ...
    def get_new_entries_since(self, timestamp):
        """Get new log entries after specified timestamp"""
        ...
        for i in range(len(self.log_entries) - 1, -1, -1):
            # Simplified handling, assumes new entries are added continuously
            # Actual implementation might need to add timestamps to log entries
            if i >= len(self.log_entries) - 10:  # Return at most 10 latest entries
...
class LogEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        # If new target log file is created
        if not event.is_directory and event.src_path == self.monitor.log_file:
            try:
                with open(event.src_path, "r", encoding="utf-8") as file:
                    for line in file:
