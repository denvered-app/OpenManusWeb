    ...
    async def cleanup(self):
        ...
        if hasattr(self, "browser") and self.browser is not None:
            try:
                ...
                self.browser = None
                self.context = None
                self.dom_service = None  # Corrected variable name
            except Exception as e:
        ...
        if hasattr(self, "browser") and self.browser is not None:
            try:
                ...
                try:
                    ...
                    if loop.is_running():
                        # If a loop is running, log a warning and skip (to avoid runtime errors)
                        logging.warning("Event loop is running, skipping browser cleanup. This may cause resource leaks.")
                        return
                except RuntimeError:
                    # No event loop is running, can create a new one
