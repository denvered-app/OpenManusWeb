...
def create_workspace(session_id: str) -> Path:
    """Create workspace directory for the session"""
...
async def get_connected_interface(request: Request):
    """New interface entry that connects to backend (same as home page)"""
    return HTMLResponse(
        content=open(
            current_dir / "static" / "connected_interface.html", encoding="utf-8"
        ).read()
...
async def get_chat_result(session_id: str):
    ...
    # Use new log handler module to get logs
    ...
    try:
        ...
        async def ws_send(message: str):
            try:
                ...
                print(f"WebSocket failed to send message: {str(e)}")

        ThinkingTracker.register_ws_send_callback(session_id, ws_send)

        # Add log information to initial status notification
        ...
        # Get workspace name (job_id) - priority from environment variable
        job_id = None
        # First check current session's workspace association
        if "workspace" in session:
            job_id = session["workspace"]

        # If no log monitor currently exists, create one
        ...
        while session["status"] == "processing":
            await asyncio.sleep(0.2)  # Reduce check interval for better real-time performance
                        ...
                        json.dumps(
                            {
                                ...
                                # Add chat_logs field to send system logs as chat messages
                                "chat_logs": new_logs,
                            }
                        )
                    )
            ...
            if session["result"]:
                ...
                break  # Result sent, exit loop to avoid duplicate sending

        # Only send final result if loop didn't break due to result
        ...
        # Other exceptions, log but don't break application
        print(f"WebSocket error: {str(e)}")
        ThinkingTracker.unregister_ws_send_callback(session_id)


# Add LLM communication hooks at appropriate location
from app.web.thinking_tracker import ThinkingTracker


# Modify communication tracker implementation method
class LLMCommunicationTracker:
    """Track communication with LLM, using monkey patching instead of callbacks"""
...
# Import newly created LLM wrapper
                ...
                workspace_item["files"].append(
                    {
                        ...
                        "type": Path(file.path).suffix[1:],  # Extension without dot
...
async def get_log_content(log_name: str, parsed: bool = False):
    ...
    # If requesting parsed log information
    if parsed:
...
async def get_latest_log():
    """Get parsing information for latest log file"""
...
async def process_prompt(session_id: str, prompt: str):
    ...
    async def sync_logs():
        """Periodically get logs from LogFileMonitor and update ThinkingTracker in real-time"""
        last_count = 0
        try:
            while True:
                ...
                if len(current_logs) > last_count:
                    ...
                    # Process each new log entry individually for real-time performance
                    for log_entry in new_logs:
                        # Process each log individually, add to ThinkingTracker immediately
                ...
                # Reduce polling interval for better real-time performance
    ...
    # Set environment variable to tell logger to use this log file, ensure both ways are set
    ...
    try:
        ...
        with capture_session_logs(session_id) as log:
            ...
            # Initial check if already cancelled
            if cancel_event and cancel_event.is_set():
                log.warning("Processing cancelled by user")
                ThinkingTracker.mark_stopped(session_id)
                active_sessions[session_id]["status"] = "stopped"
        ...
        print(error_msg)
        ThinkingTracker.add_error(session_id, f"Processing encountered an error: {str(e)}")
        ...
        # Clear log file environment variables
        ...
        if (
            ...
            try:
                ...
                print(f"Error cleaning up callbacks: {str(e)}")
