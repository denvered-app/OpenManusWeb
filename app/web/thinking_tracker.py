...
class ThinkingTracker:
    """Thinking tracker, used to record and manage AI thinking process"""

    # Class variables, store thinking steps for all sessions
            ...
            if session_id in cls._session_progress:
                ...
                if progress["total_steps"] > 0:
                    progress["percentage"] = min(
                        ...
                        99,  # Maximum 99%, only 100% when completed
                    )
    ...
    def add_log_entry(cls, session_id: str, entry: Dict) -> None:
        ...
        with cls._lock:
            ...
            # Automatically add corresponding thinking step based on log level, handle log content in more detail
            msg = entry.get("message", "")
            if entry.get("level") == "INFO":
                # Generate more meaningful thinking steps for specific types of log content
                if "Start execution" in msg:
                    cls.add_thinking_step(
                        session_id, f"Start executing task: {msg.replace('Start execution: ', '')}"
                    )
...
WRITING_STEPS = [
    "Collect relevant materials on the writing topic...",
    "Conceptualize content outline and key points...",
    "Draft initial content...",
    "Refine arguments and fact check...",
    "Polish language and formatting...",
]

# Mapping from task type to predefined steps
TASK_TYPE_STEPS = {
...
def generate_thinking_steps(
    ...
    if task_description:
        specific_steps = [
            f"Research information related to {task_description}",
            f"Analyze key points of {task_description}",
            f"Organize solutions for {task_description}",
        ]
    ...
    # Simulate adding a thinking step every so often
    for step in steps:
        ...
        if show_communication:
            ...
            ThinkingTracker.add_communication(session_id, "Sent to LLM", f"Please help me {step}...")

            # Simulate receiving LLM response
            ThinkingTracker.add_communication(
                session_id, "Received from LLM", f"I have completed {step}. Here are the relevant results: [Detailed information]"
            )

    # Add conclusion
    ThinkingTracker.add_conclusion(session_id, "Task processing completed! Results have been generated.")