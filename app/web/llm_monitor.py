"""
LLM communication monitoring module, used to capture and simulate communication with LLM
"""
import asyncio
import random
import time
...
class LLMMonitor:
    ...
    def register_interceptor(self, func: Callable):
        ...
        return func  # Allow use as a decorator
...
# Provide some functions to simulate LLM, can be used for demos or testing
async def simulate_llm_thinking(
    prompt: str, callback: Optional[Callable] = None, steps: int = 5, delay: float = 1.0
):
    """Simulate LLM thinking process, generate a series of thinking steps"""
    # Record input
    monitor.record_communication("in", prompt)
    
    thinking_steps = ["Analyze problem requirements", "Retrieve relevant knowledge", 
                     "Organize information", "Draft initial answer", 
                     "Check and optimize answer", "Generate final response"]
    
    # Adjust thinking steps based on prompt
    if "code" in prompt or "programming" in prompt:
        thinking_steps = ["Understand code requirements", "Design code structure", 
                         "Write core functions", "Implement error handling", 
                         "Test code functionality", "Optimize code efficiency"]
