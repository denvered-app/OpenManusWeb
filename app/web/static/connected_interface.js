...
class App {
    ...
    updateDynamicTexts() {
        ...
        } else if (statusIndicator.textContent.includes('Processing stopped')) {
    ...
    handleWebSocketMessage(data) {
        ...
        // Handle system logs - convert system logs to thinking steps and display
        if (data.system_logs && data.system_logs.length > 0) {
            console.log('Received system logs:', data.system_logs);

            // Convert system logs to thinking step format and add to thinking timeline
        ...
        if (data.chat_logs && data.chat_logs.length > 0) {
            console.log('Received chat logs:', data.chat_logs);
        }

        // If processing is complete, refresh workspace files
